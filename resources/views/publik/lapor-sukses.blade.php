...
<head>
    ...
    <title>Laporan Terkirim — SIOPK Badung</title>
    ...
    <style>
        :root { --tanah:#2C1A0E; --emas:#C8922A; --emas-muda:#E8B84B; --hijau:#2D5A27; }
        body { font-family:'Inter',sans-serif; background:#f4f0e8; min-height:100vh; display:flex; align-items:center; justify-content:center; }
        .sukses-card { background:white; border-radius:6px; max-width:520px; width:90%; border-top:4px solid var(--hijau); box-shadow:0 8px 30px rgba(0,0,0,0.1); overflow:hidden; }
        .sukses-header { background:linear-gradient(135deg,var(--hijau),#4a8a43); padding:2rem; text-align:center; color:white; }
        .sukses-icon { font-size:3rem; margin-bottom:0.5rem; }
        ...
        .kode-box { background:#f4f0e8; border:1px solid #d4c9b8; border-radius:3px; padding:1rem; text-align:center; margin-bottom:1.5rem; }
        .kode-val { font-family:'Courier New',monospace; font-size:1.3rem; font-weight:700; color:var(--tanah); letter-spacing:0.1em; }
        .kode-hint { font-size:0.72rem; color:#9ca3af; margin-top:4px; }
        .info-row { display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid #f0ebe3; font-size:0.82rem; }
        .info-row:last-child { border-bottom:none; }
        .info-key { color:#9ca3af; }
        .info-val { font-weight:500; color:var(--tanah); }
...
<div class="sukses-card">
    ...
    <div class="sukses-body">
        <div class="kode-box">
            <div style="font-size:0.68rem;color:#9ca3af;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">Kode Laporan Anda</div>
        ...
        <div class="d-flex gap-3">
            ...
            <a href="{{ route('publik.lapor.status') }}?kode_laporan={{ $laporan->kode_laporan }}"
               style="flex:1;text-align:center;border:1px solid #d4c9b8;padding:10px 24px;border-radius:3px;text-decoration:none;color:var(--tanah);font-size:0.88rem;">
